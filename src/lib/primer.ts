...
export const PRIMER: PrimerCard[] = [
	{
		...
		lede: "A model repo holds two kinds of weights: the ones nothing can regenerate, and the ones a script can.",
		body: [
	...
	},
	{
		...
		body: [
			"Full fidelity is sixteen bits a weight — BF16 or F16, two bytes a parameter. Quantization rounds those weights into eight, four, even two bits, trading a little accuracy for a lot of memory: a 27B model is 55 GB at BF16 and about 16 GB at Q4_K_M. The names encode the recipe. `Q4_K_M` is llama.cpp's 4-bit K-quant, medium; `IQ2_XS` a 2-bit importance-weighted one; `FP8`, `AWQ`, `GPTQ` and `NF4` are other families.",
	...
	},
	{
		...
		body: [
			"**safetensors** is the Hub's native format: a JSON header naming every tensor with its shape and dtype, then raw bytes. Big models shard into `model-00001-of-00028.safetensors` with a `model.safetensors.index.json` that says which shards make the loadable set. darsay reads that index and each header — a few dozen kilobytes per file — to classify a repo without downloading it.",
			"**GGUF** is llama.cpp's single-file container: a key/value header (architecture, context length, quantization level, and `quantize.imatrix.*` keys when an importance matrix was used) followed by the tensors. One repo often ships twenty of them at different levels — a pack. Nobody wants the pack; `--include '*Q4_K_M*'` prices and archives one.",
	...
	},
	{
		...
		body: [
			"Every row with a safetensors header shows its parameter count and dominant dtype — `27.78B · BF16`. Multiply the width by the count and you have what one copy of the weights should weigh. `BF16` and `F16` are two bytes a parameter; `F32` four; `F8_E4M3` one; packed integer quants (`U8`, or `I32` holding INT4) come in under one.",
			"So a 27.78B BF16 model should weigh about 55.6 GB. When the repo weighs 239.7 GB it is not a bigger model — it ships several weight sets. `estimate` flags that ratio at 1.75× as `redundant`, and `classify` lays the sets out.",
			"A native FP8 or INT4 release — DeepSeek, Kimi K2 — is not a downgrade. It is how the model was trained and shipped, which makes it the master.",
		],
		table: {
			...
			rows: [
				["F32", "4", "111 GB"],
				["BF16 · F16", "2", "55.6 GB"],
				["F8_E4M3 · I8 · U8", "1", "27.8 GB"],
				["INT4, packed", "0.5", "13.9 GB"],
			],
		},
		collect:
	...
	},
	{
		...
		body: [
			...
			"Rerunning `archive` on the same source continues that pin — same files, same selection. That is why resume needs no special subcommand, and why a 700 GB job can be seventy evenings of `--max-gb 10`. It never chases a moving `main`; to take a new snapshot, `--force` pins again.",
	...
	},
	{
		...
		body: [
			...
			"Above a few hundred gigabytes, think in disks and people. `assemble --move` hands a fetched half to the drive that has room and leaves a skeleton behind, so the laptop never re-fetches what it gave away. `--shard 1/2` lets two collectors prefer different halves and merge offline.",
	...
	},
	{
		...
		body: [
			"A gated repo answers 401 to anonymous requests. Accept the terms once on the model page, run `hf auth login` once on the machine, and every darsay verb works as before — the gate is enforced server-side; darsay carries your token, it does not forge one.",
			"Gating has a cost the board can see. darsay cannot read a gated file's header without the token, so on a gated repo `estimate` prices the whole thing and `classify` marks weight sets `unknown` — which means fetched, never skipped.",
		],
	...
	},
	{
		...
		body: [
			"The technique follows a 2024 finding that refusal in chat models is mediated by a single direction in activation space. Find that direction with a few hundred harmful and harmless prompts, project it out of the weight matrices, and the model stops declining without any fine-tuning — its knowledge is still the base model's; only the reflex is gone. Community releases carry it in the name: *abliterated*, *OBLITERATED*, *uncensored*, *heretic*.",
			...
			"They also tend to be messy repos: several weight sets, GGUFs whose headers name a build that was never published, provenance that disagrees with itself. `classify` refuses to guess in exactly those cases and fetches everything undecidable; the `redundant` chip is often the first sign.",
			"This lens reads the repo name, not the bytes.",
		],
		collect:
	...
	},
	{
		...
		body: [
			...
			"For the collector the first number is the one that matters. The vault holds every expert, because the router may call any of them; RAM to run it is nearer the total too — darsay's preflight compares free memory to weights × 1.2. The second number is why a 397B model can feel like a 17B one once it is loaded.",
			"This lens reads the repo name, not the bytes; MoE models without the `-AxxB` suffix are not counted.",
		],
		collect: "Budget disk by the total, and do not mistake “only 17B active” for a small download.",
	...
	},
	{
		...
		body: [
			...
			"This lens reads the repo name — eagle, medusa, speculator, draft, mtp — not the bytes.",
		],
		collect:
	...
	},
	{
		...
		body: [
			...
			"Datasets are how a model's knowledge is re-created rather than copied. A vault with weights but no data can run the models it has; a vault with data can train the next ones. Licenses matter more here, and the manifest records what upstream declared.",
		],
	...
	},
	{
		...
		body: [
			"Desire runs 1–9 and orders the board. It is also what `darsay archive --next` reads: the highest-desire source you do not have yet is the next one it fetches. The note is the sentence that justifies the number — future you, and the friend who adopts this list, will read it.",
			"Have means at least one vault on this board holds a complete, verified bundle. Who says which — a name, a drive, a city. Reporting done from the CLI flips the row to have and, if who is empty, fills in the client that finished it.",
		...
		cmd: {
			...
			lines: ["darsay catalog adopt summer ./summer.json", "darsay archive --next summer --max-gb 10"],
		},
	...
	},
	{
		...
		lede: "When a collector's CLI is fetching a row, the row says so — who, how far, and since when.",
		body: [
			"`darsay archive --next <board-url>` claims the row it picks before the first byte moves, then reports progress at file boundaries. The board draws the gold gauge; other members see the row is spoken for, and their `--next` skips it.",
			"A claim goes stale when the client stops reporting — a closed laptop, a lost network — and another collector may take the row over. Reporting done flips the row to have. Claims live on the board only; the exported catalog never carries them.",
		],
		collect:
			"Two people fetching the same 700 GB is the failure this exists to prevent. Let the CLI claim; edit have and who by hand only when the CLI could not.",
		cmd: {
