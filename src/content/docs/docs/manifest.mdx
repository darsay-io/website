...
description: "The manifest is the recorded facts about a bundle. Unknown is null. The tool never fabricates."
editUrl: "https://github.com/darsay-io/darsay/edit/main/docs/MANIFEST.md"
---
...
| `subset.policy` | `"negatives"` when the selection was made by default classification: retain all weights and support except weight sets whose every file has a hash-identical retained same-bundle twin ([Quantization §4](/docs/quantization/#4-mechanics)); absent for explicit `--include` subsets. It names the preservation policy, not the verdict for every retained byte. Duplicate paths are omitted; their content remains. |
...
| `precision` | The release precision label — `BF16`, `FP8`, `MXFP4`, `AWQ INT4`, `Q4_K_M` — from `config.json`'s `quantization_config` (wherever a multimodal config nests it), else the dominant tensor dtype, else a GGUF-only payload's file name ([Quantization §2](/docs/quantization/#2-precision-and-size-scope)). Precision does not establish original-release status. |
