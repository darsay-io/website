---
title: "catalog.json — schema reference (v3.0.0)"
description: "A catalog is a curated list of works. The vault is that list, realized. Possession is a view, not a rewrite of this file."
editUrl: "https://github.com/darsay-io/darsay/edit/main/docs/CATALOGS.md"
---
...
| `precision` | The release precision label — `BF16`, `FP8`, `MXFP4`, `AWQ INT4`, `Q4_K_M` — from `config.json`'s `quantization_config`, the dominant dtype, or (GGUF-only repos) the file name ([Quantization §2](/docs/quantization/#2-precision-and-size-scope)); `null` when nothing establishes it. Precision does not establish original-release status. |
