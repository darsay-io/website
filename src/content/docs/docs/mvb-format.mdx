...
applies the same refusals (container major, schema major, an existing
destination without `--force`), prints the bundle id, destination, and
payload size, and unpacks nothing. `darsay export BUNDLE --dry-run` likewise
